...
def main():
    ...
    mainDataFolder = "F:\\temp\\tr\\data_flag_4"
    #expFolder = "A_16_T_1600_N_1_d_2_alpha_0.01"
    expFolder = "_T_2000_CV_2_d_2"
    #expFolder = "A_16_T_1600_N_1_d_2_alpha_0.5"

    fullExperimentFolderPath = os.path.join(mainDataFolder,expFolder)
    ...
    for t in range(0, numberOfTrials):
        ...
        for file in filesInTheFolder:
            ...
            data_Vec_Est.append(results[file][1][t])
            data_Vec_Rew_Est.append(results[file][2][t])
            data_Vec_LinUCB.append(results[file][3][t])
    ...
    ax1.plot(t_Range, mean_KTL_Est,lw=4, label='KTL_Est',color='blue')    
    ax1.plot(t_Range, mean_KTL_Rew_Est,lw=4, label='KTL_Rew_Est',color='red')   
    ax1.plot(t_Range, mean_KTL_LinUCB,lw=4, label='LinUCB',color='green')   
    ...
    ax3.set_title("LinUCB vs KTL_Est")
    ax3.plot(t_Range, mean_KTL_LinUCB,lw=4, label='LinUCB',color='green') 
    ax3.fill_between(t_Range, lower_KTL_LinUCB, upper_KTL_LinUCB, facecolor='green', alpha=0.5)  
    ax3.plot(t_Range, mean_KTL_Est,lw=4, label='KTL_Est',color='blue')
    ax3.fill_between(t_Range, lower_KTL_Est, upper_KTL_Est, facecolor='blue', alpha=0.5)
